...
public class LegalConsultationCommandServiceImpl implements LegalConsultationCommandService {
    ...
    public Long handle(CreateLegalConsultationCommand command) {
        ...
        var legalConsultation = new LegalConsultation(
                command.medicId().profileId(),
                command.lawyerId().profileId(),
                command.consultationNotes()
        );
        try {
    ...
    public Optional<LegalConsultation> handle(UpdateLegalConsultationCommand command) {
        ...
        try {
            var updatedConsultation = legalConsultationRepository.save(
                    consultationToUpdate.updateInformation(command.date(), command.consultationNotes())
            );
